...
module.exports.forgotPassword = async (req, res) => {
    try {
        ...
        if (!req.session.adminId) {
            req.session.errorMessage = "Session expired. Please start again.";
            return res.redirect('/');
        }

        if (!newPass || !ConfPass) {
            req.session.errorMessage = "Please fill both password fields.";
            return res.redirect('/forgot-pass');
        }
        ...
        delete req.session.adminId;
        delete req.session.OTP;
        req.session.successMessage = "✅ Password changed successfully! Login now.";
        return res.redirect('/');

    } catch (err) {
        console.log("Error:", err.message);
        req.session.errorMessage = "Failed to update password. Try again.";
        return res.redirect('/forgot-pass');
    }
}
